      ...
      messages: [
        {
          role: "system",
          content:
            "You improve exactly one resume bullet and return only the improved bullet.",
        },
...
export async function handleEnhanceBullet(req: Request) {
  try {
...
/** @deprecated Use POST /api/v2/generate/resume/enhance-bullet */
export async function POST(req: Request) {
  const response = await handleEnhanceBullet(req);
  return addLegacyEndpointDeprecation(
    response,
    "/api/v2/generate/resume/enhance-bullet",
  );
}
