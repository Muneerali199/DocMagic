import {
  CheckCircle,
  PresentationIcon as LayoutPresentationIcon,
  Users,
  PenTool,
  Download,
  Sparkles,
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="mx-auto max-w-4xl text-center mb-10 sm:mb-16 md:mb-20">
          ...
          <p className="modern-body text-base sm:text-lg md:text-xl lg:text-2xl text-muted-foreground max-w-3xl mx-auto leading-relaxed px-4">
            Experience the future of document creation. Our AI understands context, follows best practices, and delivers
              ...
              <div key={feature.name} className={`group relative animate-slide-in-left ${animationDelay} will-change-transform`}>
                <div
                  className={`absolute inset-0 bg-gradient-to-r ${
                    theme.gradientClass.includes("sunset")
                      ? "from-amber-400/10 to-orange-400/10"
                      : theme.gradientClass.includes("ocean")
                      ? "from-blue-400/10 to-cyan-400/10"
                      : theme.gradientClass.includes("forest")
                      ? "from-emerald-400/10 to-teal-400/10"
                      : "from-purple-400/10 to-pink-400/10"
                  } rounded-2xl sm:rounded-3xl blur-xl group-hover:blur-2xl transition-all duration-300`}
                ></div>
                ...
                <div className={`relative professional-card mobile-card p-5 sm:p-6 md:p-7 lg:p-8 rounded-2xl sm:rounded-3xl hover:scale-[1.02] sm:hover:scale-105 transition-all duration-500 ${theme.cardClass} hover:${theme.glowClass} border ${theme.borderClass}`}>
                  ...
                  <div className="inline-flex items-center gap-2 px-3 sm:px-4 py-2 rounded-full glass-effect border border-current/20 touch-target">
                    ...
                    <span className="text-sm font-medium bolt-gradient-text">{feature.badge}</span>
                  </div>
                </div>
              </div>
            );
        ...
        <div className="text-center mt-12 sm:mt-16 md:mt-20">
          <Link
            href="/auth/signin"
            className="inline-flex items-center gap-3 sm:gap-4 px-6 sm:px-8 py-3 sm:py-4 rounded-full glass-effect border border-green-200/30 hover:scale-105 transition-transform duration-300 touch-target"
          >
