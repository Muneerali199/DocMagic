  ...
  >
    <defs>
      ...
      >
        ...
        <path
          d="M0 10h8M12 10h8M10 0v8M10 12v8"
          stroke={color}
          strokeWidth="0.5"
          opacity="0.2"
        />
...
function normalizeVisualType(value: unknown): CodeVisualType | "" {
  ...
  if (["svg", "svg_code", "svgcode", "vector"].includes(type))
    return "svg_code";
  if (["mermaid", "diagram", "logic"].includes(type)) return "mermaid";
  if (["html", "html_tailwind", "tailwind", "mockup"].includes(type))
    return "html_tailwind";
  if (["chart", "chart_data", "data", "recharts"].includes(type))
    return "chart_data";
...
function getRenderableMermaidCode(markup: string, slide: Slide): string {
  ...
  const laneWordCount = (
    normalized.match(/\b(outcome lane|execution lane|strategy lane)\b/gi) || []
  ).length;
...
function applyThemeToMarkupColors(
  markup: string,
  theme: PresentationTheme,
): string {
  const map: Array<[RegExp, string]> = [
    [
      /#(?:4f46e5|6366f1|3b82f6|0ea5e9|38bdf8|2563eb|1d4ed8)/gi,
      theme.colors.accent,
    ],
    [
      /#(?:10b981|22c55e|16a34a|059669)/gi,
      mixHex(theme.colors.accent, "#22c55e", 0.35),
    ],
    [
      /#(?:f59e0b|f97316|fb923c|fbbf24)/gi,
      mixHex(theme.colors.accent, "#f59e0b", 0.4),
    ],
    [
      /#(?:ef4444|dc2626|b91c1c)/gi,
      mixHex(theme.colors.accent, "#ef4444", 0.35),
    ],
    [/#(?:ffffff|fff|f8fafc|f9fafb)/gi, theme.colors.card],
    [
      /#(?:0f172a|111827|1f2937|334155|000000|020617)/gi,
...
function forceThemeOnHtml(markup: string, theme: PresentationTheme): string {
  ...
  result = result
    ...
    .replace(
      /(border\s*:\s*)([^;"']+)/gi,
      (_full, prefix: string) => `${prefix}1px solid ${theme.colors.border}`,
    )
    .replace(
      /(box-shadow\s*:\s*)([^;"']+)/gi,
      (_full, prefix: string) =>
        `${prefix}0 14px 30px ${toRgba(theme.colors.foreground, 0.16)}`,
    );
...
function buildPremiumSvgIllustration(
  slide: Slide,
  theme: PresentationTheme,
): string {
...
function getRenderableSvgMarkup(
  markup: string,
  slide: Slide,
  theme: PresentationTheme,
): string {
...
type PremiumMockArchetype =
  | "executive_dashboard"
  | "product_workflow"
  | "ops_command_center";

function inferPremiumMockArchetype(
  slide: Slide,
  seed: number,
): PremiumMockArchetype {
  const text =
    `${slide.title || ""} ${slide.subtitle || ""} ${slide.content || ""}`.toLowerCase();
  if (/(mobile|app|onboarding|funnel|signup|journey|retention)/.test(text))
    return "product_workflow";
  if (
    /(ops|operation|pipeline|incident|monitor|control|security|command)/.test(
      text,
    )
  )
    return "ops_command_center";
  return seed % 3 === 1
    ? "product_workflow"
    : seed % 3 === 2
      ? "ops_command_center"
...
function scoreHtmlMockupMarkup(markup: string): number {
  ...
  const blockCount = countMarkupMatches(
    text,
    /<div\b|<section\b|<article\b|<aside\b|<header\b/g,
  );
  const hasHeader = /<header\b|toolbar|top bar/.test(text);
  const hasLeftNav = /sidebar|side nav|navigation|<aside\b/.test(text);
  const hasKpiRow = /kpi|metric|revenue|retention|arr|growth|conversion/.test(
    text,
  );
...
function buildPremiumHtmlMockup(
  slide: Slide,
  theme: PresentationTheme,
): string {
...
function getRenderableHtmlMockup(
  markup: string,
  slide: Slide,
  theme: PresentationTheme,
): string {
...
function parseChartData(value: unknown): Slide["chartData"] | undefined {
  ...
  const chartTypeRaw =
    typeof source.type === "string" ? source.type.toLowerCase().trim() : "bar";
  const chartType = ["bar", "line", "pie", "area", "radar", "funnel"].includes(
    chartTypeRaw,
  )
...
function sanitizeVisualPayload(
  ...
  if (visualType === "chart_data") {
    if (typeof visualContent === "string" && visualContent.trim().length > 0)
      return visualContent.trim();
    if (
      visualContent &&
      typeof visualContent === "object" &&
      !Array.isArray(visualContent)
    ) {
...
function normalizeSlideRecord(rawSlide: any, index: number): Slide {
  ...
  const visualType = normalizeVisualType(
    rawSlide.visual_type || rawSlide.visualType,
  );
  ...
  const chartData = parseChartData(
    isFirstSlide
      ...
      : rawSlide.chartData ||
          rawSlide.charts ||
          (visualType === "chart_data" ? normalizedVisualContent : null),
  );
  ...
  const title =
    typeof rawSlide.title === "string" && rawSlide.title.trim()
      ? rawSlide.title.trim()
      : `Slide ${index + 1}`;
  const subtitle =
  ...
  return {
    slideNumber:
      Number(rawSlide.slideNumber) > 0
        ? Number(rawSlide.slideNumber)
        : index + 1,
    ...
    bullets,
    cta:
      typeof rawSlide.cta === "string"
        ? rawSlide.cta.trim() || undefined
        : undefined,
    design: {
    ...
    imageUrl:
      typeof rawSlide.imageUrl === "string"
        ? rawSlide.imageUrl
  ...
  return (rawSlides || []).map((rawSlide, index) => {
    ...
    return {
      ...
      chartData:
        ...
        (normalized.visualType === "chart_data"
          ? {
              type: "bar",
              data: [
                { name: "Q1", value: 10 },
      ...
      content:
        safeContent ||
        (safeBullets.length === 0 && !hasRenderableVisual
          ? "Key insight"
          : safeContent),
    };
  });
...
function computeLayoutProfile({
  ...
  const map: Record<LayoutArchetype, Omit<SlideLayoutProfile, "archetype">> = {
    ...
    "text-visual": {
      ...
      titleClass:
        contentLen > 520 ? "text-3xl md:text-4xl" : "text-4xl md:text-5xl",
      subtitleClass: "text-lg md:text-2xl",
      bodyClass:
        contentLen > 680 ? "text-base md:text-lg" : "text-lg md:text-xl",
      bulletClass:
        contentLen > 680 ? "text-base md:text-lg" : "text-lg md:text-xl",
...
export default function RealTimeGenerator() {
  ...
  const [imageGeneratorSlideIndex, setImageGeneratorSlideIndex] = useState<
    number | null
  >(null);
    ...
    async function loadPresentation() {
      ...
      try {
        ...
        if (error || !data) {
          ...
          if (legacyData) {
            ...
            if (error || legacyError)
              console.error("Load error:", error || legacyError);
          }
        }
        ...
        if (data) {
          ...
          const loadedThemeId =
            content.themeId || storedSlides.themeId || "peach";
  ...
  const [activeTab, setActiveTab] = useState<
    "all" | "dark" | "light" | "colorful" | "professional"
  >("all");
  ...
  const filteredThemes = PRESENTATION_THEMES.filter((t) => {
    const matchesSearch = t.name
      .toLowerCase()
      .includes(searchTheme.toLowerCase());
  ...
  const handleAddImageToSlide = (imageUrl: string, imageType: string) => {
    if (imageGeneratorSlideIndex !== null) {
      ...
      logger.info(
        null,
        `✅ Added ${imageType} image to slide ${imageGeneratorSlideIndex + 1}`,
      );
    }
  ...
  const navigatePresentSlides = useCallback(
    (step: number) => {
  ...
  const jumpToPresentSlide = useCallback(
    (target: number) => {
      if (!slides.length) return;
  ...
  const handleSavePresentation = async (isAutoSave = false) => {
    ...
    try {
      ...
      logger.info(
        null,
        isAutoSave ? "🤖 Presentation auto-saved" : "✅ Presentation saved:",
        result?.id,
      );
    } catch (error) {
  ...
  useEffect(() => {
    ...
    setPresentSlideIndex((prev) =>
      Math.max(0, Math.min(prev, slides.length - 1)),
    );
  }, [slides.length]);
  ...
  const handleGenerateOutline = async () => {
    ...
    try {
      ...
      alert(
        `Failed to generate outline: ${err instanceof Error ? err.message : "Unknown error"}. Please try again.`,
      );
    } finally {
      setIsGeneratingOutline(false);
    }
  ...
  const handleExport = async (format: "png" | "pdf" | "pptx") => {
    ...
    try {
      ...
      const normalizedExportSlides = normalizeSlidesForExport(
        slides,
        currentTheme,
      );
      ...
      if (process.env.NODE_ENV === "development") {
        const diagnostics = slideElements.map((el, idx) => {
          ...
          const titleRect = el
            .querySelector("[data-slide-title]")
            ?.getBoundingClientRect();
          const bodyRect = el
            .querySelector("[data-slide-body]")
            ?.getBoundingClientRect();
          const visualRect = el
            .querySelector("[data-slide-visual]")
            ?.getBoundingClientRect();
          const bulletsRect = el
            .querySelector("[data-slide-bullets]")
            ?.getBoundingClientRect();
          return {
            slide: idx + 1,
            ...
            regions: {
              ...
              visual: visualRect
                ? {
                    w: Math.round(visualRect.width),
                    h: Math.round(visualRect.height),
                  }
                : null,
              bullets: bulletsRect
                ? {
                    w: Math.round(bulletsRect.width),
                    h: Math.round(bulletsRect.height),
                  }
                : null,
            },
          };
      ...
      await exportPremiumPresentation(
        ...
        currentTheme,
        {
          ...
          targetSize:
            format === "pdf" ? { width: 1920, height: 1080 } : undefined,
        },
      );
      ...
      if (pastedText.includes("---")) {
        ...
        const response = await fetch("/api/generate/presentation-outline", {
          ...
          body: JSON.stringify({
            ...
            pageCount: Math.min(
              12,
              Math.max(1, Math.ceil(pastedText.length / 200)),
            ),
            outlineOnly: true,
          }),
        });
    ...
    <div className="min-h-screen bg-background relative overflow-hidden font-sans text-foreground selection:bg-blue-500/30">
        ...
        <div className="max-w-7xl mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            ...
            >
              ...
              <div>
                <h1 className="text-lg font-bold professional-heading tracking-tight">
                  DraftDeckAI
                </h1>
              </div>
            </div>
      ...
      <div className="pt-24 pb-32 relative z-10">
          ...
          <div className="max-w-6xl mx-auto px-6">
            <div className="text-center mb-16 pt-10">
              ...
              <h2 className="text-5xl md:text-6xl font-bold professional-heading mb-6 tracking-tight">
                Create with{" "}
                <span className="bolt-gradient-text">Intelligence</span>
              </h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              >
                ...
                <div className="bg-card/60 backdrop-blur-xl w-full h-full rounded-[20px] p-6 flex flex-col relative overflow-hidden border border-border hover:border-indigo-500/50 shadow-lg hover:shadow-indigo-500/10 transition-all">
                  ...
                  <h3 className="text-xl font-bold professional-heading mb-2 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">
                    Webpage
                  </h3>
                  <p className="text-sm text-muted-foreground leading-relaxed">
                    Turn any URL into a deck.
                  </p>
                </div>
              </button>
            </div>
          ...
          <div className="min-h-[calc(100vh-8rem)] flex flex-col items-center justify-center px-6 py-12">
            <div className="max-w-3xl w-full animate-fade-in-up">
              <div className="text-center mb-8">
                <h2 className="text-4xl md:text-5xl font-bold professional-heading mb-4 tracking-tight">
                  Transform a{" "}
                  <span className="bolt-gradient-text">Webpage</span>
                </h2>
                <p className="text-xl text-muted-foreground">
                  Paste a URL to turn any article or blog post into a
                  presentation.
                </p>
              </div>
            ...
            <div className="max-w-4xl w-full animate-fade-in-up">
              <div className="text-center mb-8">
                <h2 className="text-4xl md:text-5xl font-bold professional-heading mb-4 tracking-tight">
                  ...
                  <span className="bolt-gradient-text">create?</span>
                </h2>
              </div>
              ...
              <div className="flex flex-wrap items-center justify-center gap-4 mb-8">
                <div className="flex items-center gap-3 bg-card px-4 py-2 rounded-xl border border-border shadow-sm">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      onClick={() =>
                        setSlideCount(Math.min(20, slideCount + 1))
                      }
                      className="w-8 h-8 rounded-lg bg-muted hover:bg-muted/80 flex items-center justify-center transition-colors"
                      type="button"
                    >
                    ...
                    <div className="flex flex-col items-center gap-8 relative z-10">
                      ...
                      <div className="text-center space-y-3">
                        ...
                        <p className="text-muted-foreground">
                          Creating {slideCount} expert-crafted cards for{" "}
        ...
        {view === "outline-review" && (
          <div className="max-w-7xl mx-auto px-6 flex flex-col lg:flex-row gap-8">
            ...
            <div className="w-full lg:w-80 flex-shrink-0 space-y-6 h-fit sticky top-24 overflow-y-auto max-h-[calc(100vh-8rem)] pr-2">
              <h3 className="text-lg font-bold professional-heading mb-4">
                Settings
              </h3>
                    ...
                    {PRESENTATION_THEMES.slice(0, 6).map((t) => (
                      ...
                      >
                        ...
                        >
                          ...
                          >
                            <div
                              className="h-1.5 w-1/3 rounded-full mb-2 opacity-20"
              ...
              <div className="w-1/3 border-r border-border flex flex-col bg-muted/30">
                <div className="p-6 border-b border-border space-y-4">
                  ...
                  <div className="flex gap-2 overflow-x-auto pb-2 custom-scrollbar">
                    {["all", "dark", "light", "colorful", "professional"].map(
                      (tab) => (
                        <button
                          key={tab}
                    ...
                    {filteredThemes.map((t) => (
                      ...
                      >
                        ...
                        >
                          ...
                          >
                            <div
                              className="h-2 w-1/3 rounded-full mb-3 opacity-20"
                              style={{ backgroundColor: t.colors.foreground }}
                            />
        ...
        <AIImageGeneratorModal
          ...
          slideTitle={
            imageGeneratorSlideIndex !== null &&
            slides[imageGeneratorSlideIndex]
              ? slides[imageGeneratorSlideIndex].title
              : ""
          }
          slideContent={
            imageGeneratorSlideIndex !== null &&
            slides[imageGeneratorSlideIndex]
              ? slides[imageGeneratorSlideIndex].content
              : ""
          }
          slideType={
            imageGeneratorSlideIndex !== null &&
            slides[imageGeneratorSlideIndex]
              ? slides[imageGeneratorSlideIndex].type
              : "content"
          }
        ...
        {isPresenting && (
          ...
          >
            ...
            <div className="h-full w-full flex items-center justify-center p-6 md:p-8">
              <div
                className="aspect-video w-full max-h-[88vh] shadow-2xl mx-auto"
                style={{ maxWidth: "calc(88vh * 16 / 9)" }}
              >
                <AnimatePresence
                  mode="wait"
                  initial={false}
                ...
                >
                  <motion.div
                    ...
                    transition={getSlideMotionTransition(
              ...
              {slides.length === 0 && isStreaming && (
                <div className="relative overflow-hidden flex flex-col items-center justify-center min-h-[60vh] animate-in fade-in zoom-in-50 duration-500 rounded-3xl border border-border/60 bg-card/40">
                  ...
                  <p className="text-lg text-muted-foreground max-w-md text-center leading-relaxed">
                    DraftDeckAI is crafting your slides, designing layouts, and
                ...
                <div key={index} className="animate-fadeIn" data-slide-card>
                  <SlideCard
                    ...
                    onUpdate={(updatedSlide) =>
                      handleSlideUpdate(index, updatedSlide)
                    }
              ...
              {isStreaming && currentSlideText && slides.length > 0 && (
                <div className="relative animate-pulse">
                  ...
                  <div className="bg-card rounded-2xl sm:rounded-3xl p-6 sm:p-8 md:p-12 aspect-video flex flex-col items-center justify-center border border-border shadow-xl">
                    ...
                    <h3 className="text-xl font-bold professional-heading mb-2">
                      Creating Slide {slides.length + 1}
                    </h3>
                    <p className="text-muted-foreground max-w-md text-center">
                      Analyzing content and designing layout...
                    </p>
                  </div>
                </div>
              )}
              ...
              <div className="p-8">
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3 pt-4">
                    ...
                    <button
                      onClick={() =>
                        handleCopyLink(`Check out my presentation: ${shareUrl}`)
                      }
                      className="px-4 py-3 bg-card hover:bg-muted border border-border rounded-xl font-medium transition-all flex items-center justify-center gap-2"
                    >
                      <FileText className="w-4 h-4" />
          ...
          <div className="fixed bottom-0 left-0 right-0 bg-background/80 backdrop-blur-xl border-t border-border p-4 z-50 animate-slideUp">
            <div className="max-w-7xl mx-auto flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div className="text-sm text-muted-foreground">
                  Est. time:{" "}
                  <span className="text-foreground font-bold">~1min</span>
                </div>
              </div>
...
export function SlideCard({
  ...
  const chartData =
    slide.chartData ||
    (visualType === "chart_data" ? parseChartData(visualContent) : undefined);
  const hasCodeVisual =
    !isFirstSlide &&
    (visualType === "svg_code" ||
      visualType === "mermaid" ||
      visualType === "html_tailwind");
  const hasChart =
    !isFirstSlide && chartData && chartData.data && chartData.data.length > 0;
  const hasImage =
    slide.imageUrl && slide.imageUrl.length > 0 && !hasCodeVisual;
  const hasStats = slide.stats && slide.stats.length > 0;
  const hasComparison =
    slide.comparison &&
    (slide.comparison.left?.length > 0 || slide.comparison.right?.length > 0);
  ...
  const supportsDiagramSourceEditor =
    isEditable && ["html_tailwind", "mermaid", "svg_code"].includes(visualType);
  const isHtmlInlineEditable =
    isEditable && visualType === "html_tailwind" && !isPresentMode;
  ...
  const parseIconBullet = (
    text: string,
  ): {
    IconComponent: React.FC<{ className?: string; color?: string }> | null;
    text: string;
    iconName: string;
  } => {
    ...
    const emojiMatch = text.match(
      /^([\u{1F300}-\u{1F9FF}]|[\u{2600}-\u{26FF}]|[\u{2700}-\u{27BF}]|✓|•|⚡|🛡️|👥|🌍|🎯|🚀|❤️|⭐|📈|⏰|🔒|🏆|💡|📊|💰|📱|☁️|💻|🎨)\s*/u,
    );
    if (emojiMatch) {
  ...
  const textColor =
    getOptimalTextColor(theme.colors.background) || theme.colors.foreground;
  ...
  const getTotalContentLength = () => {
    ...
    if (slide.bullets)
      len += slide.bullets.reduce((acc, b) => acc + b.length, 0);
    return len;
  };
    ...
    >
      ...
      >
        ...
        <div className="absolute inset-0 overflow-hidden">
          <div
            className={`min-h-full w-full ${layoutProfile.shellPaddingClass} flex flex-col items-center justify-center relative z-10`}
          >
            <div
              className={`${layoutProfile.contentWidthClass} ${layoutProfile.textAlignClass} w-full relative z-10`}
            >
              {/* Icon for slide type */}
              {!isHero && (
                <div
                ...
                animatePresentText(
                  <p
                    ...
                    onBlur={(e) =>
                      onUpdate?.({
                        ...slide,
                        subtitle: e.currentTarget.textContent || slide.subtitle,
                      })
                    }
              ...
              {hasCodeVisual &&
                animatePresentText(
                  <div
                    data-slide-visual
                    className={layoutProfile.visualShellClass}
                  >
                    ...
                    >
                      ...
                      {visualType === "svg_code" && themedSvgMarkup && (
                        <div
                          className="h-full rounded-xl overflow-auto p-3 [&>svg]:w-full [&>svg]:h-auto"
                          style={{ backgroundColor: theme.colors.card }}
                          dangerouslySetInnerHTML={{ __html: themedSvgMarkup }}
                        />
                      )}
                      {visualType === "html_tailwind" && themedHtmlMarkup && (
                        <div
                          className="h-full rounded-xl overflow-hidden p-2"
                          style={{ backgroundColor: theme.colors.card }}
                        >
                          <div
                            className={`h-full w-full origin-top ${isPresentMode ? "scale-[0.96]" : "scale-[0.9]"}`}
                          >
                            <div
                              ...
                              onBlur={(e) => {
                                ...
                                const editedMarkup = stripThemeVisualWrapper(
                                  e.currentTarget.innerHTML || "",
                                );
                                if (editedMarkup)
                                  updateDiagramVisualContent(editedMarkup);
                              }}
                              dangerouslySetInnerHTML={{
                                __html: themedHtmlMarkup,
                              }}
                            />
                          </div>
                        </div>
                    ...
                    {supportsDiagramSourceEditor && !isPresentMode && (
                      <div
                        data-export-hide="true"
                        className="mt-3 rounded-xl border border-border/70 bg-card/80 p-3"
                      >
                        <div className="flex items-center justify-between gap-2">
                          ...
                          >
                            ...
                            {isDiagramEditorOpen
                              ? "Hide Diagram Text Editor"
                              : "Edit Diagram Text"}
                          </button>
                          {visualType === "html_tailwind" && (
                            <span className="text-[11px] text-muted-foreground">
                              Tip: You can also click inside the mockup to edit
                              text.
                            </span>
                          )}
                        </div>
              ...
              {slide.stats && slide.stats.length > 0 && (
                <div
                  data-slide-visual
                  className={`grid gap-6 mt-10 ${slide.stats.length === 2 ? "grid-cols-2" : slide.stats.length === 3 ? "grid-cols-3" : "grid-cols-2 md:grid-cols-4"}`}
                >
              ...
              {slide.mockup && (
                <div data-slide-visual className="mt-10 flex justify-center">
                          ...
                          <div className="space-y-3 flex-1">
                            {slide.mockup.elements.map((el, idx) => (
                              <div key={idx}>
                                ...
                                {el.type === "input" && (
                                  <div
                                    className="px-3 py-2 rounded-lg border text-sm opacity-60"
                                    style={{
                                      borderColor: `${textColor}30`,
                                      color: textColor,
                                    }}
                  ...
                  {slide.mockup.type === "laptop" && (
                    <div className="relative">
                      <div className="w-[600px] h-[380px] rounded-t-xl border-8 border-gray-800 bg-gray-900 shadow-2xl overflow-hidden">
                        ...
                        >
                          <div
                            className="h-8 border-b flex items-center gap-2 px-4"
                  ...
                  {slide.mockup.type === "browser" && (
                    <div
                      className="w-full max-w-4xl rounded-xl border-2 overflow-hidden shadow-2xl"
                      style={{ borderColor: `${textColor}30` }}
                    >
                      <div
                        className="h-10 border-b flex items-center gap-2 px-4"
                        style={{
                          borderColor: `${textColor}20`,
                          backgroundColor: `${textColor}05`,
                        }}
                      >
                        ...
                        <div
                          className="flex-1 mx-4 h-6 rounded-full px-3 flex items-center text-sm"
                          style={{
                              ...
                              {el.type === "nav" && (
                                ...
                                >
                                  {el.content.split(",").map((item, i) => (
                                    <span key={i} className="hover:opacity-100">
                                      {item.trim()}
                                    </span>
                                  ))}
                                </div>
              ...
              {slide.icons && slide.icons.length > 0 && (
                <div
                  data-slide-visual
                  className={`grid gap-6 mt-10 ${slide.icons.length <= 3 ? "grid-cols-3" : slide.icons.length === 4 ? "grid-cols-4" : "grid-cols-3 md:grid-cols-6"}`}
                >
                  {slide.icons.map((item, idx) => {
                    // Try to get SVG icon from the icon name or emoji
                    const iconName =
                      typeof item.icon === "string"
                        ? item.icon.replace(/[^\w]/g, "")
                        : "";
              ...
              {slide.testimonial && (
                <div data-slide-visual className="mt-10 max-w-3xl mx-auto">
                  ...
                  >
                    <span
                      className="absolute -top-6 -left-2 text-8xl opacity-20"
                      style={{ color: theme.colors.accent }}
                    >
                      "
                    </span>
                    <p
                      className="text-2xl md:text-3xl font-medium italic leading-relaxed mb-8 relative z-10"
                      style={{ color: textColor }}
                    >
                ...
                (() => {
                  ...
                  if (layout === "full") {
                    return (
                      <div
                        data-slide-visual
                        className="mt-8 mb-8 flex justify-center w-full"
                      >
                        <div
                        ...
                        >
                          ...
                          <img
                            ...
                            style={{
                              maxHeight: layoutProfile.imageMaxHeight,
                              minHeight: "220px",
                            }}
                          />
                          <div
                ...
                animatePresentText(
                  <p
                    ...
                    onBlur={(e) =>
                      onUpdate?.({
                        ...slide,
                        content: e.currentTarget.textContent || slide.content,
                      })
                    }
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="flex-1 pt-1">
                              <span
                                className={`${layoutProfile.bulletClass} leading-relaxed font-medium`}
                              >
                                {parsed.text}
                              </span>
                            </div>
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex-1 pt-2">
                            <span
                              className={`${layoutProfile.bulletClass} font-medium leading-relaxed opacity-95`}
                              style={{ color: textColor }}
                            >
                              {parsed.text}
                            </span>
              ...
              {isFlowchart &&
                ...
                animatePresentText(
                  <div
                    data-slide-bullets
                    className="flex flex-col md:flex-row items-center justify-center gap-6 mt-10 flex-wrap"
                  >
